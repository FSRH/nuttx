...
#if BOARD_NGPIOIN > 0 || BOARD_NGPIOOUT > 0
...
#if BOARD_NGPIOIN > 0
static const struct gpio_operations_s gpin_ops = { .go_read = gpin_read,
		.go_write = NULL, .go_attach = NULL, .go_enable = NULL, };
...
static const struct gpio_operations_s gpint_ops =
		{ .go_read = gpint_read, .go_write = NULL, .go_attach = gpint_attach,
...
static int gpout_read(FAR struct gpio_dev_s *dev, FAR bool *value) {
	...
	DEBUGASSERT(stm32gpio != NULL && value != NULL);DEBUGASSERT(stm32gpio->id < BOARD_NGPIOOUT);
	gpioinfo ("Reading...\n");
...
static int gpout_write(FAR struct gpio_dev_s *dev, bool value) {
	...
	DEBUGASSERT(stm32gpio != NULL);DEBUGASSERT(stm32gpio->id < BOARD_NGPIOOUT);
	gpioinfo ("Writing %d\n", (int) value);
